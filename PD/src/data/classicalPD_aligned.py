...
if __name__ == "__main__":
    ...
    # Lorenz curve
    plt.figure(1)
    plt.plot(analysis_OoS['Cum Bad'], analysis_OoS['Cum Good'], label='Validation set' + ' (area = %0.4f)' % auc_OsS)
    plt.plot(analysis_IS['Cum Bad'], analysis_IS['Cum Good'], label='Training set' + ' (area = %0.4f)' % auc_IS)
    ...
    plt.title('Kolmogorov-Smirnov (Validation)')
    plt.legend()
    ...
    plt.title('Kolmogorov-Smirnov (Training)')
    plt.legend()
    plt.subplots_adjust(bottom=0.25)
    plt.show()
